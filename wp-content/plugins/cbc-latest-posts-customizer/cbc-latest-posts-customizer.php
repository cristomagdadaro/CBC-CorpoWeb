<?php
/**
 * Plugin Name: CBC Latest Posts Customizer
 * Plugin URI: https://cropbiotechcenter.da.gov.ph
 * Description: Preserves custom modifications to the core latest-posts block across WordPress updates
 * Version: 1.0.0
 * Author: Christ Rey C. Magdadaro
 * License: MIT
 * Text Domain: cbc-latest-posts
 ...
 * This incorporates all custom modifications from latest-posts.php
 * and will persist across WordPress updates.
 *
 * @param array $attributes The block attributes.
 *
 * @return string Returns the post content with latest posts added.
 */
function cbc_render_latest_posts( $attributes ) {
	...
	$render_item = function( $post, $attributes, $container_classes, $image_wrapper_template, $show_image = true, $is_right = false ) use ( $is_grid_layout, &$block_core_latest_posts_excerpt_length ) {
		...
		if (!$is_grid_layout) {
			$meta  = get_post_meta( $post->ID, 'pm_metrics', true );
			$views = intval( $meta['views'] ?? 0 );
			$item_markup .= '<div class="flex items-start sm:items-center gap-1"><span class="!text-xs !font-spartan">' . $views . '</span><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-eyeglasses" viewBox="0 0 16 16">
...
/**
 * Unregister the core latest-posts block and re-register it with our custom render callback.
 * This ensures our customizations persist across WordPress updates.
 */
function cbc_override_latest_posts_block() {
	unregister_block_type( 'core/latest-posts' );

	register_block_type_from_metadata(
		ABSPATH . 'wp-includes/blocks/latest-posts',
		array(
			'render_callback' => 'cbc_render_latest_posts',
		)
	);
}
add_action( 'init', 'cbc_override_latest_posts_block', 20 );
