(function (_$) {
	...
	document.addEventListener('DOMContentLoaded', function () {
		...
		function selectPoster(idx) {
			const frame = mediaFrame({ title: 'Select poster image', button: 'Use poster', multiple: false, types: 'image' });
			frame.on('select', function () {
		...
		function replaceMedia(idx, types) {
			...
			frame.on('select', function () {
				...
				if (type === 'image') { patch.alt = media.alt || ''; patch.posterId = 0; patch.posterUrl = ''; }
				...
				<div class="cbc-slide" data-idx="${idx}">
					...
					<div class="cbc-slide__form">
						...
						${isVideo ? `
							...
							<div class="cbc-row">
								<button type="button" class="button" data-action="poster">Select Poster</button>
								${slide.posterUrl ? `<span class="cbc-note">Poster set</span>` : ''}
							</div>
						` : `
							<label>Alt text <input type="text" data-field="alt" value="${escAttr(slide.alt||'')}"></label>
						`}
						<label>Overlay position
							<select data-field="overlayPosition">
								${[
									...
									{v:'center',l:'Center'}
								].map(o => `<option value="${o.v}" ${slide.overlayPosition===o.v?'selected':''}>${o.l}</option>`).join('')}
							</select>
						</label>
						<label>Overlay HTML
		...
		function render() {
			...
			if (addImagesBtn) addImagesBtn.addEventListener('click', function () { addFromMedia('image', true, 'image'); });
			const addVideosBtn = root.querySelector('[data-action="add-videos"]');
			if (addVideosBtn) addVideosBtn.addEventListener('click', function () { addFromMedia('video', true, 'video'); });
