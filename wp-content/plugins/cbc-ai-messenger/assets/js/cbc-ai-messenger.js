(function ($) {
    ...
    function initChatWidget() {
        ...
        if (!collapsed && isMobile()) {
            applyMobileFullscreen($container, $panel);
        }
        ...
        fetch(CBCAI.restUrl, {
            ...
            .then(function (data) {
                if (data && data.reply) {
                    ...
                    addMsg($panel, 'bot', 'Error: ' + data.error);
                } else {
            ...
            .catch(function () {
                addMsg($panel, 'bot', 'Network error. Please try again.');
            })
            .finally(function () {
    ...
    $(document).on('click', '#cbc-ai-chat-panel .cbc-ai-edit-user', function () {
        var $panel = $('#cbc-ai-chat-panel');
        try {
            localStorage.removeItem('cbc_ai_user_name');
            localStorage.removeItem('cbc_ai_user_email');
        } catch (e) {
        }
        hideUserInfo($panel);
    });
