 ...
 * at render time and used by the block itself.
 *
 * @var int
 */
 ...
 *
 * @return int Returns the global excerpt length for the block.
 */
function cbc_latest_posts_get_excerpt_length() {
	global $block_core_latest_posts_excerpt_length;
	return $block_core_latest_posts_excerpt_length;
}

/**
 * Custom render callback for core/latest-posts block.
 *
 ...
 */
function cbc_render_latest_posts( $attributes, $content = '', $block = null ) {
	...
	add_filter( 'excerpt_length', 'cbc_latest_posts_get_excerpt_length', 20 );
	...
	$render_item = function( $post, $attributes, $container_classes, $image_wrapper_template, $show_image = true, $is_right = false ) use ( $is_grid_layout, &$block_core_latest_posts_excerpt_length ) {
		...
		$item_markup .= '<a class="wp-block-latest-posts__post-title text-normal md:text-lg uppercase !font-sans text-left font-bold !leading-none md:leading-relaxed z-[99] group-hover:[text-shadow:1px_1px_0_white,-1px_1px_0_white,1px_-1px_0_white,-1px_-1px_0_white,0_2px_0_white,2px_0_0_white,-2px_0_0_white,0_-2px_0_white]" href="' . esc_url( $post_link ) . '">' . esc_html( $title ) . '</a>';

		$item_markup .= '<div class="flex justify-between text-xs md:text-base">';
		$item_markup .= "<div class='flex " . ( isset( $attributes['displayAuthor'] ) && $attributes['displayAuthor'] ? 'flex-col items-start' : 'flex-row w-full justify-between' ) . " sm:flex-row sm:items-center gap-0 sm:gap-2'>";
		...
		if ( ! $is_grid_layout ) {
			...
			$item_markup .= '<div class="flex items-start sm:items-center gap-1"><span class="!text-xs !font-spartan">' . $views . '</span><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-eyeglasses" viewBox="0 0 16 16"><path d="M4 6a2 2 0 1 1 0 4 2 2 0 0 1 0-4m2.625.547a3 3 0 0 0-5.584.953H.5a.5.5 0 0 0 0 1h.541A3 3 0 0 0 7 8a1 1 0 0 1 2 0 3 3 0 0 0 5.959.5h.541a.5.5 0 0 0 0-1h-.541a3 3 0 0 0-5.584-.953A2 2 0 0 0 8 6c-.532 0-1.016.208-1.375.547M14 8a2 2 0 1 1-4 0 2 2 0 0 1 4 0"/></svg></div>';
		}
		...
		if ( isset( $attributes['displayPostContent'] ) && $attributes['displayPostContent'] && isset( $attributes['displayPostContentRadio'] ) && 'excerpt' === $attributes['displayPostContentRadio'] ) {
			...
			$excerpt_base       = 'wp-block-latest-posts__post-excerpt !m-0 text-xs md:text-base !font-spartan !leading-none md:!leading-5 entry-content';
			$excerpt_visibility = ( $is_grid_layout && $is_right ) ? 'hidden' : 'hidden md:block';
	...
	if ( ! empty( $attributes['uniqueID'] ) ) {
		$right_list_id .= sanitize_html_class( (string) $attributes['uniqueID'] );
	}
	...
	foreach ( $recent_posts as $post ) {
		...
		if ( $is_grid_layout ) {
			...
			if ( ! $is_first_group ) {
				...
				$list_items_markup .= $render_item( $post, $attributes, $container_classes . 'hover:shadow-lg', $img_wrapper_first, true );
			}
		}
	...
	remove_filter( 'excerpt_length', 'cbc_latest_posts_get_excerpt_length', 20 );
...
/**
 * Force the core latest-posts block to use the custom renderer.
 *
 ...
 */
function cbc_override_latest_posts_block_args( $args, $block_name ) {
	if ( 'core/latest-posts' === $block_name ) {
		$args['render_callback'] = 'cbc_render_latest_posts';
	}

	return $args;
}
add_filter( 'register_block_type_args', 'cbc_override_latest_posts_block_args', 10, 2 );

/**
 * Handles outdated versions of the core/latest-posts block by converting
 * the categories attribute from a numeric string to an array with key id.
 *
 * @param array $block A single parsed block object.
 *
 * @return array The migrated block object.
 */
function cbc_latest_posts_migrate_categories( $block ) {
	if (
...
add_filter( 'render_block_data', 'cbc_latest_posts_migrate_categories' );