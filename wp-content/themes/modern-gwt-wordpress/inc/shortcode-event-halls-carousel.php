 ...
 *
 * Usage: [event_halls_carousel]
 */

if (!function_exists('cbc_event_halls_carousel_shortcode')) {
    function cbc_event_halls_carousel_shortcode($atts) {
        $atts = shortcode_atts(array(
            ...
            'hall' => 'plenary', // Default hall
        ), $atts, 'event_halls_carousel');

        // Define event halls data
        $event_halls = array(
            'plenary' => array(
                array(
                        ...
                        'name' => 'The Plenary Hall',
                        'image' => '/wp-content/uploads/2025/10/CBC_20210920_0042-scaled.jpg',
                ),array(
                        'name' => 'The Plenary Hall',
                        'image' => '/wp-content/uploads/2025/10/CBC_20210920_0044-scaled.jpg',
                ),array(
                        'name' => 'The Plenary Hall',
                        'image' => '/wp-content/uploads/2025/10/CBC_20210920_0045-scaled.jpg',
                ),
            ),
            'mph' => array(
                array(
        ...
        $plenary_data = apply_filters('cbc_event_halls_data', $event_halls['plenary']);
        $mph_data = apply_filters('cbc_event_halls_data', $event_halls['mph']);
        $training_data = apply_filters('cbc_event_halls_data', $event_halls['training']);

        if (empty($plenary_data) && empty($mph_data) && empty($training_data)) {
            return '<p>No Event Halls available.</p>';
        }
...
<h1>update</h1>
            <div class="cbc-carousel relative overflow-hidden "
                ...
                <div class="cbc-carousel-track">
                    ...
                    switch ($atts['hall']) {
                        case 'mph':
                            $event_halls = $mph_data;
                            break;
                        case 'training':
                            $event_halls = $training_data;
                            break;
                        default:
                            $event_halls = $plenary_data;
                            break;
                    }

                    foreach ($event_halls as $index => $commodity): ?>
                        ...
                        >
                            ...
                            <div class="cbc-carousel-item-content">
                                <span class="cbc-carousel-item-title hidden">
    ...
    add_shortcode('event_halls_carousel', 'cbc_event_halls_carousel_shortcode');
}
