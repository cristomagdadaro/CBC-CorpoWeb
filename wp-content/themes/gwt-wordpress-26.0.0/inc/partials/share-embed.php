...
?>
<div class="gwt-share-btns-container" data-post-id="<?php echo esc_attr( $post->ID ); ?>">
  <div class="gwt-share-buttons" role="group" aria-label="Share">
    <p class="flex items-center">Share via </p>
    <button class="gwt-share-btn gwt-share-facebook" data-share="facebook" data-pm-label="shortcode-share" data-pm-event="share" title="Share to Facebook">
        <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="h-auto w-6 text-white" viewBox="0 0 16 16">
    ...
    <button class="gwt-share-btn gwt-share-embed" data-share="embed" data-pm-label="shortcode-share" data-pm-event="share" title="Get embed code">
        <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="h-auto w-6 text-gray-900" viewBox="0 0 16 16">
  ...
  document.addEventListener('click', function(e){
    ...
    if (action === 'facebook') {
      ...
      if (navigator.clipboard && clipboardWriteSupported()) {
        navigator.clipboard.writeText(decodeURIComponent(permalink)).then(function(){
          alert('Link copied to clipboard');
        }).catch(function(){
          prompt('Copy this link', decodeURIComponent(permalink));
        });
      } else {
        try { prompt('Copy this link', decodeURIComponent(permalink)); } catch (ex) { alert('Copy this link: ' + decodeURIComponent(permalink)); }
      }
      ...
      if (navigator.clipboard && clipboardWriteSupported()) {
        navigator.clipboard.writeText(embedHtml).then(function(){
          ...
          prompt('Copy this embed code', embedHtml);
        });
      } else {
        try { prompt('Copy this embed code', embedHtml); } catch (ex) { alert('Embed code:\n' + embedHtml); }
      }
    }
  }, false);
